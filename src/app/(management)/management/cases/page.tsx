...
export default function ManagementCasesPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-4">
      <h1 className="text-lg font-semibold" style={{ color: "var(--content-strong)" }}>Sprzedaże w strukturze</h1>
      {cases.length === 0 ? (
        ...
        cases.map((c: any) => (
          <Card key={c.id} className="cursor-pointer hover:opacity-90" onClick={() => router.push(`/cases/${c.id}`)}>
            <CardContent className="flex items-center justify-between py-3">
              <div>
                <p className="text-sm font-medium" style={{ color: "var(--content-strong)" }}>{c.client?.companyName} — {c.product?.name || "brak produktu"}</p>
                <p className="text-xs" style={{ color: "var(--content-muted)" }}>{c.assignedTo?.name || "—"}</p>
              </div>
