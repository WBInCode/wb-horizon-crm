        ...
        cases.map((c: any) => (
          <Card key={c.id} className="cursor-pointer hover:opacity-90" onClick={() => router.push(`/cases/${c.id}`)}>
            <CardContent className="flex items-center justify-between py-3">
              <div>
                <p className="text-sm font-medium" style={{ color: "var(--content-strong)" }}>{c.title}</p>
                <p className="text-xs" style={{ color: "var(--content-muted)" }}>{c.client?.companyName}</p>
              </div>
              <Badge variant="outline">{c.processStage || c.status}</Badge>
            </CardContent>
          </Card>
        ))
      )}
