...
import { Users, Calendar, LayoutDashboard, LogOut, BookOpen } from "lucide-react"
import { signOut } from "next-auth/react"

const ccMenu = [
  { label: "Dashboard", href: "/cc", icon: LayoutDashboard },
      ...
      >
        <div className="px-5 py-5" style={{ borderBottom: "1px solid var(--sidebar-border)" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-semibold" style={{ color: "var(--sidebar-accent-foreground)" }}>Call Center</h1>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {ccMenu.map((item) => {
            ...
            const isActive = item.href === "/cc" ? pathname === "/cc" : pathname.startsWith(item.href)
            return (
              <Link
                key={item.href}
                href={item.href}
