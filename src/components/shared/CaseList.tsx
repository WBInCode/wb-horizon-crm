...
interface CaseListProps {
  ...
  getSalespersonName: (c: CaseItem) => string | null | undefined
}

export function CaseList({ fetchUrl, title, productFallback = "—", getSalespersonName }: CaseListProps) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-4">
      <h1 className="text-lg font-semibold" style={{ color: "var(--content-strong)" }}>{title}</h1>
      {cases.length === 0 ? (
            ...
            <CardContent className="flex items-center justify-between py-3">
              <div>
                <p className="text-sm font-medium" style={{ color: "var(--content-strong)" }}>
                  {c.client?.companyName} — {c.product?.name || productFallback}
                </p>
                <p className="text-xs" style={{ color: "var(--content-muted)" }}>
                  {getSalespersonName(c) || "—"}
                </p>
              </div>
